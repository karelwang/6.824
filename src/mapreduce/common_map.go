...
func doMap(
	...
	intermediateKV := mapF(inFile, string(fileContent))

	tmpFilePrefix := "mrtmp." + jobName + "-" + strconv.Itoa(mapTaskNumber) + "-"
	a := make([]int, nReduce)
	encs := make([]*json.Encoder, nReduce)
	for i := range a {
		fd, err := os.Create(tmpFilePrefix + strconv.Itoa(i))
		if err != nil {
			log.Fatal(err)
		}
		encs[i] = json.NewEncoder(fd)
	}
